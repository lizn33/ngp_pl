...
def parse_args():
	...
	parser.add_argument("--video_fps", default=2.0)
	...
	parser.add_argument("--sparse", default="sparse", help="Input path to the colmap sparse files (set automatically if --run_colmap is used).")
	...
	parser.add_argument("--resolution", default="1920:-1", help="Resolution of the video.")
...
def run_ffmpeg(args):
	...
	do_system(f"{ffmpeg_binary} -i {video} -qscale:v 1 -qmin 1 -vf \"fps={fps}{time_slice_value},scale={scale}\" {images}/%04d.jpg")
	
def run_colmap(args):
	colmap_binary = "colmap"
	images = "\"" + args.images + "\""
	if not os.path.isabs(args.colmap_db):
		args.colmap_db = os.path.join(os.path.dirname(args.images), args.colmap_db)
	db =  "\"" + args.colmap_db + "\""
	if not os.path.isabs(args.colmap_db):
		args.text = os.path.join(os.path.dirname(args.images), args.text)
	text =  "\"" + args.text  + "\""
	if not os.path.isabs(args.sparse):
		args.sparse = os.path.join(os.path.dirname(args.images), args.sparse)
	sparse =  "\"" + args.sparse + "\""
